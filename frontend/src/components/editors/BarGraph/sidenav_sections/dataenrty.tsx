...
export const DataPanel: React.FC<DataPanelProps> = ({ data, setData }) => {
  ...
  const handleItemChange = (index: number, key: "name" | "value", value: string) => {
    const newData = [...data];
    ...
    <div>
      {data.map((item, index) => (
        <div key={index} style={cardStyle}>
          <h3 style={{ marginBottom: "0.75rem", color: "#0077ff" }}>Item {index + 1}</h3>

          <label style={labelStyle}>
            <div style={{ marginBottom: "0.3rem", color: "#ff4fa3" }}>Item Name</div>
            <input
              type="text"
              value={item.name}
          ...
          <label style={labelStyle}>
            <div style={{ marginBottom: "0.3rem", color: "#ff4fa3" }}>Quantity/Value</div>
            <input
              type="number"
              value={item.value}
